...
输出格式要求：
1. 按重要性排序，分类标注：🔥 热门、📊 论文/观点、🛠 工具/开源、💼 行业动态
2. 每条包含：序号、标题、1-2句中文摘要、原文链接
3. 原文链接必须使用 Markdown 格式：[来源名称](URL)，不要贴裸链接
4. 最后用一句话总结今日AI领域的整体动向
...
class Summarizer:
    ...
    def generate_digest(self, articles: list[dict], top_n: int = 15) -> str:
        article_text = ""
